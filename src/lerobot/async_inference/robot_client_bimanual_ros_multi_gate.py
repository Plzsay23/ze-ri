...
class RobotClient:
    ...
    def control_loop_observation(self, task: str, verbose: bool = False) -> RawObservation:
        try:
            ...
            try:
                raw_observation["active_arm"] = self._get_active_arm()
            except Exception:
                pass
...
class BimanualRosMultiPolicyGateRobotClient(RobotClient):
    """ROS-gated multi-policy bimanual robot client.

    This runs one LeRobot RobotClient with robot.type=bi_so_follower so shared
    cameras such as /dev/top are opened only once. It receives ROS commands,
    selects policy_id, and streams arm/task metadata to the policy server.

    If the loaded policy is a true bimanual policy, action tensors should match
    len(robot.action_features) and are applied normally. If the policy returns
    a single-arm action tensor, this client can map it to the selected arm and
    hold the other arm at the current observed pose. This fallback only works if
    the policy server accepts the configured feature schema.

    This keeps the normal LeRobot RobotClient running, but replaces
    ...
    def __init__(self, config: RobotClientConfig):
        ...
        self.zeri_client_name = os.environ.get("ZERI_CLIENT_NAME", "bimanual").strip() or "bimanual"
        self.zeri_command_topic = os.environ.get("ZERI_COMMAND_TOPIC", "/zeri/vla/bimanual/command")
        self.zeri_stop_topic = os.environ.get("ZERI_STOP_TOPIC", "/zeri/vla/bimanual/stop")
        self.zeri_status_topic = os.environ.get("ZERI_STATUS_TOPIC", "/zeri/vla/bimanual/status")
    ...
    def _handle_command_text(self, raw_text: str) -> None:
        ...
        arm = self._normalize_arm(str(command.get("arm") or command.get("target_arm") or "").strip())
    ...
    @staticmethod
    def _normalize_arm(value: str) -> str | None:
        value = (value or "").strip().lower()
        if value in {"left", "l", "left_arm", "왼팔"}:
            return "left"
        if value in {"right", "r", "right_arm", "오른팔"}:
            return "right"
        if value in {"", "none", "null", "both", "bimanual", "dual"}:
            return None
        raise ValueError(f"Invalid arm: {value}. Use left/right/both.")

    def _get_active_arm(self) -> str | None:
        with self._active_lock:
            return self._active_arm

    def _split_action_keys_by_arm(self) -> tuple[list[str], list[str], list[str]]:
        ...
        for key in keys:
            lk = key.lower()
            # Common bi_so_follower naming styles:
            # left_arm.shoulder_pan.pos / left.shoulder_pan.pos / l_shoulder_pan.pos
            if (
                "left" in lk
                or lk.startswith("l_")
                or lk.startswith("l.")
        ...
        # If the robot exposes bimanual action keys without left/right labels,
        # assume first half is left and second half is right.
        if not left and not right and len(keys) % 2 == 0:
            half = len(keys) // 2
            left = keys[:half]
            right = keys[half:]
            unknown = []
    ...
    def _latest_current_action_dict(self) -> dict[str, float] | None:
        latest_obs = self._get_latest_raw_observation()
        if latest_obs is not None:
            current = self._extract_action_dict_from_observation(latest_obs)
            if current is not None:
                return current

        try:
            obs = self.robot.get_observation()
            self._update_latest_raw_observation(obs)
            return self._extract_action_dict_from_observation(obs)
        except Exception as e:
            self.logger.warning(f"[bimanual-gate] failed to get current pose for inactive arm hold: {e}")
            return None

    def _action_tensor_to_action_dict(self, action_tensor: torch.Tensor) -> dict[str, float]:
        """Convert server action tensor to robot action dict.

        Supports two modes:
        1. True bimanual policy: len(action_tensor) == len(robot.action_features).
        2. Single-arm policy fallback: len(action_tensor) == len(selected_arm_keys).
           In this case only the selected arm is updated and the other arm is held
           at its current observed pose.
        """
        action_keys = list(self.robot.action_features)
        flat = action_tensor.detach().cpu().flatten()

        if len(flat) == len(action_keys):
            return {key: float(flat[i].item()) for i, key in enumerate(action_keys)}

        active_arm = self._get_active_arm()
        left_keys, right_keys, unknown_keys = self._split_action_keys_by_arm()

        if active_arm == "left":
            active_keys = left_keys
        elif active_arm == "right":
            active_keys = right_keys
        else:
            raise RuntimeError(
                f"Action tensor length {len(flat)} does not match bimanual action length {len(action_keys)}, "
                "and no active arm is set. Command must include arm='left' or arm='right', "
                "or use a true bimanual policy."
            )

        if unknown_keys:
            self.logger.warning(
                f"[bimanual-gate] action keys without arm label: {unknown_keys}. "
                "Single-arm fallback may be unsafe."
            )

        if len(flat) != len(active_keys):
            raise RuntimeError(
                f"Single-arm action length mismatch. tensor_len={len(flat)}, "
                f"active_arm={active_arm}, active_keys={active_keys}, "
                f"all_action_keys={action_keys}. Use a bimanual policy or fix key mapping."
            )

        current = self._latest_current_action_dict()
        if current is None:
            raise RuntimeError("Cannot hold inactive arm because current bimanual pose could not be read.")

        action = {key: float(current[key]) for key in action_keys if key in current}
        missing = [key for key in action_keys if key not in action]
        if missing:
            raise RuntimeError(f"Current pose missing action keys for inactive hold: {missing}")

        for i, key in enumerate(active_keys):
            action[key] = float(flat[i].item())
...
def async_client(cfg: RobotClientConfig):
    ...
    client = BimanualRosMultiPolicyGateRobotClient(cfg)
