...
class RosMultiPolicyGateRobotClient(RobotClient):
    ...
    def _load_raw_home_ticks(self, path: str) -> dict[str, int] | None:
        """Load raw Present_Position tick targets for home return.

        Supported JSON formats:
          1. {"shoulder_pan": 2073, ...}
          2. {"shoulder_pan.pos": 2073, ...}
          3. {"raw_ticks": {"shoulder_pan": 2073, ...}}

        These are Feetech Present_Position ticks after calibration, not LeRobot
        normalized action values. They are used only for home return.
        """
        if not path:
            return None
        ...
        try:
            ...
            self.logger.error(f"[raw-home] failed to load {path}: {e}")
            return None

        if isinstance(payload, dict) and isinstance(payload.get("raw_ticks"), dict):
            payload = payload["raw_ticks"]

        if not isinstance(payload, dict):
            self.logger.error(f"[raw-home] invalid JSON format in {path}: expected dict")
            return None

        targets: dict[str, int] = {}
        for key, value in payload.items():
            motor = str(key).strip().removesuffix(".pos")
            try:
                targets[motor] = int(round(float(value)))
            except Exception:
                self.logger.error(f"[raw-home] invalid tick value for {key}: {value!r}")
                return None

        return targets or None

    def _get_raw_home_bus(self):
        bus = getattr(self.robot, "bus", None)
        if bus is None:
            self.logger.error("[raw-home] self.robot has no .bus attribute; cannot use raw tick home")
            return None
        return bus

    def _run_raw_home_return_blocking(self) -> None:
        """Return home by writing raw Feetech Goal_Position ticks directly.

        This bypasses robot.send_action(), so it avoids LeRobot's calibrated
        action conversion and max_relative_target clipping during home return.
        It still clamps the target to calibration range_min/range_max if those
        are available on the motor bus.
        """
        if self.home_return_running_event.is_set():
            return

        bus = self._get_raw_home_bus()
        if bus is None or not self.zeri_home_raw_ticks:
        ...
        try:
            ...
            motor_names = list(self.zeri_home_raw_ticks.keys())

            missing = [m for m in motor_names if m not in getattr(bus, "motors", {})]
            if missing:
                self.logger.error(
                    f"[raw-home] target contains unknown motors: {missing}. "
                    f"available={list(getattr(bus, 'motors', {}).keys())}"
                )
                return

            try:
                current_raw = bus.sync_read("Present_Position", motor_names, normalize=False)
            except TypeError:
                current_raw_all = bus.sync_read("Present_Position", normalize=False)
                current_raw = {m: current_raw_all[m] for m in motor_names}

            target_raw: dict[str, int] = {}
            calibration = getattr(bus, "calibration", {}) or {}
            for motor, tick in self.zeri_home_raw_ticks.items():
                tick_i = int(tick)
                cal = calibration.get(motor)
                if cal is not None:
                    min_tick = int(cal.range_min)
                    max_tick = int(cal.range_max)
                    clamped = max(min_tick, min(max_tick, tick_i))
                    if clamped != tick_i:
                        self.logger.warning(
                            f"[raw-home] clamped {motor}: {tick_i} -> {clamped} "
                            f"within [{min_tick}, {max_tick}]"
                        )
                    tick_i = clamped
                target_raw[motor] = tick_i

            steps = max(1, int(self.home_return_seconds * self.home_return_fps))
            dt = 1.0 / max(1.0, self.home_return_fps)

            self.logger.warning(
                f"[raw-home] returning home with raw ticks | seconds={self.home_return_seconds}, "
                f"steps={steps}, current={current_raw}, target={target_raw}"
            )
            ...
            for step in range(steps):
                ...
                for motor in motor_names:
                    start = float(current_raw[motor])
                    target = float(target_raw[motor])
                    goal[motor] = int(round(start + alpha * (target - start)))

                bus.sync_write("Goal_Position", goal, normalize=False)
                time.sleep(dt)

            self.logger.warning("[raw-home] home return finished. Waiting for next command.")

        except Exception as e:
            self.logger.error(f"[raw-home] raw home return failed: {e}")
    ...
    def __init__(self, config: RobotClientConfig):
        ...
        if self.zeri_home_raw_ticks:
            self.logger.warning(
                f"[raw-home] enabled | path={self.zeri_home_raw_ticks_path} | "
                f"targets={self.zeri_home_raw_ticks}"
            )
